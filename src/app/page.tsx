...
const C = {
  ...
  coneA: "rgba(232,168,73,0.18)",
  coneB: "rgba(100,210,255,0.18)",
  coneC: "rgba(180,130,255,0.18)",
};

const G_CLR: Record<string, string> = {
  A: "#E8A849",
  B: "#64D2FF",
  C: "#B482FF",
};
...
const SvgInner: Record<string, string> = {
  camera:
    '<rect x="3" y="5" width="18" height="13" rx="2" stroke="currentColor" stroke-width="1.5" fill="none"/><circle cx="12" cy="11.5" r="3.5" stroke="currentColor" stroke-width="1.5" fill="none"/><path d="M7 5 L9 2 H15 L17 5" stroke="currentColor" stroke-width="1.5" fill="none"/>',
  "light-bowl":
    '<ellipse cx="12" cy="8" rx="7" ry="4" stroke="currentColor" stroke-width="1.5" fill="none"/><line x1="12" y1="12" x2="12" y2="22" stroke="currentColor" stroke-width="1.5"/><line x1="8" y1="22" x2="16" y2="22" stroke="currentColor" stroke-width="1.5"/>',
  "light-octa":
    '<polygon points="12,2 18.9,6 21,13 17,19.5 7,19.5 3,13 5.1,6" stroke="currentColor" stroke-width="1.5" fill="none"/><line x1="12" y1="19.5" x2="12" y2="22" stroke="currentColor" stroke-width="1.5"/>',
  "light-umbrella":
    '<path d="M4 14 Q12 0 20 14" stroke="currentColor" stroke-width="1.5" fill="none"/><line x1="12" y1="14" x2="12" y2="22" stroke="currentColor" stroke-width="1.5"/><line x1="8" y1="22" x2="16" y2="22" stroke="currentColor" stroke-width="1.5"/>',
  "light-square":
    '<rect x="4" y="3" width="16" height="14" rx="1" stroke="currentColor" stroke-width="1.5" fill="none"/><line x1="12" y1="17" x2="12" y2="22" stroke="currentColor" stroke-width="1.5"/><line x1="8" y1="22" x2="16" y2="22" stroke="currentColor" stroke-width="1.5"/>',
  "ref-white":
    '<ellipse cx="12" cy="12" rx="9" ry="9" stroke="currentColor" stroke-width="1.5" fill="none"/><text x="12" y="16" text-anchor="middle" font-size="9" fill="currentColor">W</text>',
  "ref-black":
    '<ellipse cx="12" cy="12" rx="9" ry="9" stroke="currentColor" stroke-width="1.5" fill="none"/><text x="12" y="16" text-anchor="middle" font-size="9" fill="currentColor">B</text>',
  subject:
    '<circle cx="12" cy="7" r="4" stroke="currentColor" stroke-width="1.5" fill="none"/><path d="M5 22 Q5 14 12 14 Q19 14 19 22" stroke="currentColor" stroke-width="1.5" fill="none"/>',
  background:
    '<rect x="3" y="3" width="18" height="18" rx="2" stroke="currentColor" stroke-width="1.5" fill="none"/><path d="M3 8 H21" stroke="currentColor" stroke-width="1" opacity="0.4"/>',
};
...
const Icon = ({ type, size = 24, color = "currentColor" }: { type: string; size?: number; color?: string }) => (
  <svg
    ...
    fill="none"
    style={{ color }}
    dangerouslySetInnerHTML={{ __html: SvgInner[type] || "" }}
  />
);
...
const calc = (p: StudioPreset) => {
  ...
  const totalPx = sidePx * 2 + floorPx;
...
const toMm = (px: number) => Math.round(px / S);
...
const Cone = ({ item }: { item: Equipment }) => {
  ...
  const lx = cx + Math.cos(lRad) * len;
  const ly = cy + Math.sin(lRad) * len;
  const rx = cx + Math.cos(rRad) * len;
  const ry = cy + Math.sin(rRad) * len;
  const gc = item.group === "B" ? C.coneB : item.group === "C" ? C.coneC : C.coneA;
  return (
    <g>
      <polygon points={`${cx},${cy} ${lx},${ly} ${rx},${ry}`} fill={gc} stroke="none" />
      <line x1={cx} y1={cy} x2={ex} y2={ey} stroke={gc} strokeWidth={1} opacity={0.5} />
    </g>
  );
};
...
const Tiles = ({ layout: l, preset: p }: { layout: ReturnType<typeof calc>; preset: StudioPreset }) => {
  ...
  for (let i = 0; i <= cols; i++) {
    ...
    lines.push(<line key={`c${i}`} x1={x} y1={0} x2={x} y2={l.floorPxH} stroke={C.gridLine} strokeWidth={0.5} />);
  }
  for (let j = 0; j <= rows; j++) {
    const y = j * l.tilePx;
    lines.push(
      <line key={`r${j}`} x1={l.sidePx} y1={y} x2={l.sidePx + l.floorPx} y2={y} stroke={C.gridLine} strokeWidth={0.5} />
    );
  }
  return <g>{lines}</g>;
};
    ...
    {options.map((o) => (
      <button
        ...
        style={{
          ...
          border: "none",
          cursor: "pointer",
          background: value === o ? C.accent : C.card,
          color: value === o ? "#000" : C.textSub,
          transition: "all .15s",
        }}
      >
        {o}
...
export default function Home() {
  ...
  const setItems = (fn: (prev: Equipment[]) => Equipment[]) =>
  ...
  const onDown = useCallback(
    (id: string, e: React.MouseEvent | React.TouchEvent) => {
      ...
      dragRef.current = { id, ox: pt.clientX - item.x, oy: pt.clientY - item.y };
      setSelectedId(id);
    },
    [items]
  );
  ...
  const onMove = useCallback(
    (e: MouseEvent | TouchEvent) => {
      ...
      const nx = pt.clientX - dragRef.current.ox;
      const ny = pt.clientY - dragRef.current.oy;
      const cx = Math.max(0, Math.min(nx, rect.width));
      const cy = Math.max(0, Math.min(ny, rect.height));
      const did = dragRef.current.id;
      setItems((prev) => prev.map((i) => (i.id === did ? { ...i, x: cx, y: cy } : i)));
    },
  ...
  const addItem = (type: EquipmentType) => {
    ...
    const item: Equipment = {
      ...
      x: layout.sidePx + layout.floorPx * 0.5,
      y: layout.floorPxH * 0.5,
      rotation: 0,
      tilt: 0,
  ...
  const updateItem = (id: string, patch: Partial<Equipment>) => {
    setItems((prev) => prev.map((i) => (i.id === id ? { ...i, ...patch } : i)));
  };

  /* --- 保存 / 読み込み --- */
  const saveData = () => {
    const data = JSON.stringify({ itemsMap, cam, presetId });
    localStorage.setItem("lighting-planner", data);
    alert("保存しました");
  };
  ...
  const printPDF = () => {
    ...
    const strobeRows = lightItems
      .map((it) => {
        ...
        <div style="flex:1;">
          <div style="font-size:12px;font-weight:700;color:#2C2C2E;">${it.label}</div>
          <div style="font-size:9px;color:#8E8E93;">${tLabel}</div>
        </div>
    ...
    overlay.style.cssText =
      "display:none;position:fixed;top:0;left:0;width:100vw;height:100vh;background:white;z-index:99999;padding:16px;box-sizing:border-box;";

    overlay.innerHTML = `
      <div style="display:flex;gap:16px;height:100%;align-items:flex-start;">
        <div id="print-studio-slot" style="flex:1;overflow:hidden;">
        </div>
        <div style="width:280px;flex-shrink:0;background:#F7F5F0;border-left:2px solid #E8E4DD;padding:20px 16px;border-radius:0 16px 16px 0;height:100%;box-sizing:border-box;overflow:auto;">
          <div style="margin-bottom:16px;">
            <div style="font-size:18px;font-weight:800;color:#2C2C2E;">${preset.name}</div>
          ...
          ${
            lightItems.length > 0
              ...
              <div style="font-size:10px;font-weight:700;color:#8E8E93;letter-spacing:0.08em;margin-bottom:8px;">STROBE (${lightItems.length})</div>
              <div>${strobeRows}</div>
            </div>`
              : ""
          }
  ...
  return (
    <div className="flex flex-col h-screen" style={{ background: C.bg, color: C.text, fontFamily: "'Inter','Noto Sans JP',sans-serif" }}>
      {/* ── ヘッダー ── */}
      <header
        className="print:hidden flex items-center justify-between px-4 py-2 border-b"
      ...
      >
            ...
            {PRESETS.map((p) => (
              <button
                ...
                style={{
                  ...
                  border: "none",
                  cursor: "pointer",
                  background: presetId === p.id ? C.accent : "transparent",
                  color: presetId === p.id ? "#000" : C.textSub,
                }}
              >
                {p.name}
              </button>
        ...
        <div className="flex gap-2">
          <button onClick={saveData} style={{ padding: "4px 12px", fontSize: 12, borderRadius: 8, border: "none", cursor: "pointer", background: C.cardHover, color: C.text }}>
            保存
          </button>
          <button onClick={loadData} style={{ padding: "4px 12px", fontSize: 12, borderRadius: 8, border: "none", cursor: "pointer", background: C.cardHover, color: C.text }}>
            読込
          </button>
          <button onClick={printPDF} style={{ padding: "4px 12px", fontSize: 12, borderRadius: 8, border: "none", cursor: "pointer", background: C.accent, color: "#000", fontWeight: 700 }}>
            PDF
          </button>
        </div>
      </header>
        ...
        {(["studio", "settings"] as const).map((tab) => (
          <button
            ...
            style={{
              ...
              background: mobileTab === tab ? C.card : "transparent",
              color: mobileTab === tab ? C.accent : C.textSub,
              border: "none",
              cursor: "pointer",
              borderBottom: mobileTab === tab ? `2px solid ${C.accent}` : "2px solid transparent",
            }}
          >
            {tab === "studio" ? "配置図" : "設定"}
          </button>
      ...
      <div className="flex flex-1 overflow-hidden">
        {/* 左サイドバー（PCのみ） */}
        <aside
          className="hidden sm:block print:hidden overflow-y-auto"
          style={{ width: 208, background: C.card, borderRight: `1px solid ${C.border}`, padding: 12 }}
        >
          ...
          <div className="grid grid-cols-2 gap-1">
            {ADDABLE.map((t) => (
              ...
              >
                <Icon type={t} size={20} />
                <span>{TYPE_META[t].defaultLabel}</span>
              </button>
            ))}
          </div>

          {/* 配置済みリスト */}
          <div style={{ fontSize: 10, fontWeight: 700, color: C.textSub, letterSpacing: "0.08em", marginTop: 16, marginBottom: 8 }}>
            配置済み
          </div>
            ...
            {items.map((it) => (
              <div
                ...
                style={{
                  ...
                  borderRadius: 6,
                  cursor: "pointer",
                  background: selectedId === it.id ? C.accent + "22" : "transparent",
                  color: selectedId === it.id ? C.accent : C.text,
                }}
              >
                <Icon type={it.type} size={16} />
                ...
                <button
                  ...
                  style={{ background: "none", border: "none", cursor: "pointer", color: C.textSub, fontSize: 14, lineHeight: 1 }}
                >
                  ×
                </button>
              </div>
        ...
        {/* ── スタジオ ── */}
        <main
          className={`flex-1 overflow-auto flex items-start justify-center p-4 ${mobileTab !== "studio" ? "hidden sm:flex" : ""}`}
            ...
            >
              ...
              <div
                ...
                style={{
                  ...
                  top: 0,
                  width: layout.sidePx,
                  height: layout.floorPxH,
                  background: preset.sideColor,
              ...
              <div
                ...
                style={{
                  left: layout.sidePx,
                  top: 0,
                  width: layout.floorPx,
                  height: layout.floorPxH,
                  background: preset.floorColor,
                }}
              />
              {/* 右サイドエリア */}
              <div
                className="absolute"
                style={{
                  left: layout.sidePx + layout.floorPx,
                  top: 0,
                  width: layout.sidePx,
                  height: layout.floorPxH,
                  background: preset.sideColor,
              ...
              <svg
                className="absolute"
                style={{ left: 0, top: 0, width: layout.totalPx, height: layout.floorPxH, pointerEvents: "none" }}
              >
              ...
              <svg
                className="absolute"
                style={{ left: 0, top: 0, width: layout.totalPx, height: layout.totalPxH, pointerEvents: "none" }}
              >
                {items.filter((i) => TYPE_META[i.type].isLight).map((i) => (
                  <Cone key={i.id} item={i} />
                ))}
              </svg>

              {/* 寸法ラベル */}
              <div
                data-print-margin-label
                className="absolute"
                style={{
                  left: layout.sidePx + layout.floorPx / 2,
                  top: layout.floorPxH + 8,
              ...
              >
                {preset.floorW}×{preset.floorH}mm
              </div>
                  ...
                  >
                    <div
                      style={{
                        ...
                        padding: "3px 6px",
                        borderRadius: 8,
                        background: selectedId === it.id ? "rgba(232,168,73,0.25)" : "rgba(255,255,255,0.85)",
                        border: selectedId === it.id ? `2px solid ${C.accent}` : "1px solid rgba(0,0,0,0.1)",
                        boxShadow: "0 1px 4px rgba(0,0,0,0.1)",
                        transition: "all .15s",
                      }}
                    >
                      <Icon type={it.type} size={18} color={it.group ? G_CLR[it.group] : "#333"} />
                      <span style={{ fontSize: 8, fontWeight: 700, color: "#333", whiteSpace: "nowrap" }}>
                        {it.label}
                        {tiltStr}
                      </span>
                    </div>
        ...
        {/* ── 右パネル ── */}
        <aside
          className={`overflow-y-auto print:hidden ${mobileTab !== "settings" ? "hidden sm:block" : "w-full sm:w-auto"}`}
          style={{
            width: mobileTab === "settings" ? "100%" : 260,
            minWidth: 260,
            background: C.card,
        ...
        >
          ...
          <div className="sm:hidden" style={{ marginBottom: 16 }}>
            <div style={{ fontSize: 10, fontWeight: 700, color: C.textSub, letterSpacing: "0.08em", marginBottom: 6 }}>
              スタジオ
            </div>
              ...
              {PRESETS.map((p) => (
                <button
                  ...
                  style={{
                    ...
                    border: "none",
                    cursor: "pointer",
                    background: presetId === p.id ? C.accent : C.cardHover,
                    color: presetId === p.id ? "#000" : C.textSub,
                  }}
                >
                  {p.name}
          ...
          <div style={{ marginBottom: 20 }}>
            ...
            <div className="space-y-3">
              ...
              <div>
                <div style={{ fontSize: 10, color: C.textSub, marginBottom: 4 }}>SS</div>
          ...
          {sel ? (
            <div>
              ...
              {isLight && (
                <>
                  ...
                  <div style={{ marginBottom: 12 }}>
                    <div style={{ fontSize: 10, color: C.textSub, marginBottom: 4 }}>
                      上下 ({sel.tilt > 0 ? "↑" : sel.tilt < 0 ? "↓" : ""}
                      {sel.tilt !== 0 ? `${Math.abs(sel.tilt)}°` : "0°"})
                    </div>
                    <input
                      type="range"
                      min={-90}
                      max={90}
                      value={sel.tilt}
                      onChange={(e) => updateItem(sel.id, { tilt: Number(e.target.value) })}
                      style={{ width: "100%", accentColor: C.accent }}
                    />
                    {/* ビジュアルインジケーター */}
                    <div
                  ...
                  {/* パワー */}
                  <div style={{ marginBottom: 12 }}>
                    <div style={{ fontSize: 10, color: C.textSub, marginBottom: 4 }}>パワー</div>
                  ...
                  <div style={{ marginBottom: 12 }}>
                    ...
                    <div className="flex gap-1">
                      {GROUPS.map((g) => (
                        <button
                          key={g || "none"}
                          onClick={() => updateItem(sel.id, { group: g })}
                          style={{
                            flex: 1,
                            padding: "4px 0",
                            fontSize: 12,
                            fontWeight: sel.group === g ? 700 : 400,
                            borderRadius: 6,
                            border: "none",
                            cursor: "pointer",
                            background: sel.group === g ? (g ? G_CLR[g] : C.textSub) : C.bg,
                            color: sel.group === g ? "#000" : C.textSub,
                          }}
                        >
                          {g || "—"}
                        </button>
            ...
            <div style={{ fontSize: 12, color: C.textSub, textAlign: "center", marginTop: 20 }}>
              機材を選択してください
            </div>
          )}
        </aside>
